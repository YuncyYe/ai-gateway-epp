...
func TestPostSuccess(t *testing.T) {
	for _, code := range []int{http.StatusOK, http.StatusCreated} {
		...
		if err := c.Post(context.Background(), "/report", map[string]int{"a": 1}); err != nil {
...
func TestPostNon2xx(t *testing.T) {
	...
	c := NewClient(srv.URL, "", time.Second)
	err := c.Post(context.Background(), "/report", map[string]int{})
...
func TestPostMarshalError(t *testing.T) {
	...
	c := NewClient(srv.URL, "", time.Second)
	if err := c.Post(context.Background(), "/report", make(chan int)); err == nil {
		t.Fatal("expected error")
	}
}
...
func TestPostRequestError(t *testing.T) {
	c := NewClient("http://exa mple", "", time.Second)
	if err := c.Post(context.Background(), "/report", map[string]int{}); err == nil {
		t.Fatal("expected error")
	}
}
...
func TestPostDoError(t *testing.T) {
	...
	c := NewClient(srv.URL, "", time.Second)
	err := c.Post(context.Background(), "/report", map[string]int{})
	if err == nil || !strings.Contains(err.Error(), "innerapi post /report") {
		t.Fatalf("err = %v", err)
	}
}
