...
func invalidConfig(cluster string) json.RawMessage {
	return json.RawMessage(strings.ReplaceAll(string(common.EppConfig(cluster, false)),
...
// TestTC01_HotReload: an epp_config content change is picked up by the
...
func TestTC01_HotReload(t *testing.T) {
	...
	e.API.SetEppConfig(map[string]json.RawMessage{"cluster-a": configV2("cluster-a")})
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("cluster-b engine version did not change; metrics excerpt:\n%s\nepp log tail: %s",
				metricsExcerpt(text), logTail(t, e))
		}
...
func TestTC03_InvalidConfig(t *testing.T) {
	...
	e.API.SetEppConfig(map[string]json.RawMessage{"cluster-a": invalidConfig("cluster-a")})
