...
验证多 cluster 场景下热加载的隔离性：只更新 cluster-b 的 picker_config，仅 cluster-b 的引擎换入，cluster-a 的引擎版本保持不变。
...
2. mock API 已配置 assignment（cluster-a=primary、cluster-b=primary）、两个 cluster 的 picker_config v1（最小配置）、cluster_table（cluster-a=[sim a0, Weight=50]、cluster-b=[sim b0, Weight=50]）。
...
| 2 | mock 下发配置：cluster-a 仍为 v1 原文（`PickerConfig("cluster-a", false)`），cluster-b 替换为 v2（插件 `max-score` 重命名为 `max-score-v2`） | mock 下发成功 |
