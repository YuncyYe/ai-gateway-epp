...
2. mock API 已配置 cluster-a（后端 `a0`）、cluster-b（后端 `b0`）两 cluster 的 picker_config / cluster_table 最小配置，assignment 初始为 `{"cluster-a": "primary", "cluster-b": "primary"}`。
...
| # | 操作 | 预期 |
|---|---|---|
| 1 | `SetAssignment` 将 assignment 改为 `{"cluster-b": "primary"}`（移除 cluster-a） | — |
...
| 4 | 轮询（20s 观察窗）读取 mock 收到的最新一份 assignment report，检查其中是否含 `key == "cluster-a"` 的 cell | report 不再携带 cluster-a，cell 已销毁 |

## 预期结果

- cluster-a 被摘除后调度失败，cluster-b 调度正常；
- 最新 report 的 `cells[]` 中不存在 cluster-a（FR-D4 消失纳入语义、FR-H1 主备角色）；
