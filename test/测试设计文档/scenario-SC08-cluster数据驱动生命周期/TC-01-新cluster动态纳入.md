...
验证运行中的 epp 在 assignment / picker_config / cluster_table 三个 InnerAPI 视图同时新增一个 cluster 后，自动完成建 cell、编译引擎、发现端点并对外可调度，全程无需重启 epp（FR-D4、FR-M3）。
...
2. 仅下发 cluster-a：assignment（cluster-a=primary）、picker_config（cluster-a 最小配置，cluster-table-discovery 插件钉住 clusterName=cluster-a）、cluster_table（cluster-a 下 1 个 sim 后端 a0，Weight=50）。
...
| # | 操作 | 预期 |
|---|---|---|
| 1 | 新起一个 inference-sim（sim-b0，模型 `sim-model`），并将其登记为 cluster-b 的后端；mock API 全量更新三视图：assignment 增加 `cluster-b=primary`，picker_config 增加 cluster-b 最小配置，cluster_table 增加 cluster-b/sub-1 下 sim-b0（Weight=50） | mock 接口下发成功 |
...
| 3 | 读取 mock API 收到的最新一条 report 上报 | 上报内容包含 `"key":"cluster-b"`，即新 cell 已建立并进入周期上报 |

## 预期结果

- 全部步骤通过；epp 的 stderr 日志无 error 级记录（fail 时 harness 自动 dump）。
- 三视图数据驱动纳入新 cluster：建 cell、编译引擎、发现端点、可调度、进上报，epp 进程全程未重启。
