...
2. mock API 已配置 assignment 全量视图（cluster-a → 本实例 primary）、cluster_table（2 个 sim 后端 a0/b0）；epp_config 由 `WithEppConfigFn` 下发单 scorer 配置：插件链 cluster-table-discovery + `kv-cache-utilization-scorer`（名称 "kv-scorer"）+ max-score-picker，调度 profile "default" 引用 scorer 与 max-score，requestHandler 为 openai-parser。
